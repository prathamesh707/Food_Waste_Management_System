...
const NgoPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8000/api/filter-for-ngo/${userData.address.city}/${userData.address.district}`);
  ...
  return (
        ...
        <div>
          ...
          <div className="flex row mt-2">
            ...
            <div className="flex row items-end ">
              Status :- &nbsp;
              <p style={{ color: getCurrentStatus(user.status) }}>
                {user.status}
              </p>
            </div>
          </div>
    ...
    <button
            onClick={() =>handleCart(userData._id)}
            className="flex justify-end w-30  items-end fixed bottom-10 right-10 shadow-gray-600 shadow-md p-2 bg-[#561c42] rounded-md text-white mt-3 active:shadow-none"
          >
            Cart
          </button>
    </>
